...
from ..utils.provinces import PROVINCE_DATA
...
def inference():
    ...
    if number_crop:
        ...
        # Remove non-alphanumeric chars and ensure uppercase
        plate_txt = re.sub(r"[^A-Za-z0-9\-.]", "", raw).upper()
        number_conf = c

    if province_crop:
        raw, c = ocr_parseq(province_crop, app.parseq_model, app.ocr_transform, app.ocr_device)
        # Remove non-alphanumeric chars and ensure uppercase
        province_txt = re.sub(r"[^A-Za-z0-9\-.]", "", raw).upper()
        province_conf = c

    # 5. Province correction - will return uppercase province
    ...
    # Get Khmer translation for province - use single source of truth
    khmer_province = PROVINCE_DATA.get(corrected_prov, PROVINCE_DATA.get("UNREADABLE"))

    result = [{
        "plate": plate_txt,
        "detected_province": province_txt,
        "corrected_province": corrected_prov,
        "khmer_province": khmer_province,
